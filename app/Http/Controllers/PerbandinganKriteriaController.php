<?php

namespace App\Http\Controllers;

use App\Http\Livewire\PerbandinganKriteria;
use App\Models\Kriteria;
use App\Models\PerbandinganKriteriaModel;

use Illuminate\Support\Str;
use Illuminate\Http\Request;

class PerbandinganKriteriaController extends Controller
{
    public function index()
    {
        $data = [
            'title' => 'Perbandingan Kriteria',
            'kriteria' => Kriteria::get(),
            'perbandingan_kriteria' => PerbandinganKriteriaModel::get()
        ];
        // return response()->json($data);
        return view('livewire.perbandingan-kriteria', $data);
    }

    public function store(Request $request)
    {
        $jenis = 'kriteria';
        $data = $request->all();
        $JmlKriteria = count(Kriteria::get());
        $matrik = array();
        $urut     = 0;
        for ($x = 0; $x <= ($JmlKriteria - 2); $x++) {
            for ($y = ($x + 1); $y <= ($JmlKriteria - 1); $y++) {
                $urut++;
                $pilih    = "pilih" . $urut;
                $bobot     = "bobot" . $urut;
                if ($data[$pilih] == 1) {
                    $matrik[$x][$y] = $data[$bobot];
                    $matrik[$y][$x] = 1 / $data[$bobot];
                } else {
                    $matrik[$x][$y] = 1 / $data[$bobot];
                    $matrik[$y][$x] = $data[$bobot];
                }

                inputDataPerbandinganKriteria($x, $y, $data[$pilih], $matrik[$x][$y]);
            }
        }

        // diagonal --> bernilai 1
        for ($i = 0; $i <= ($JmlKriteria - 1); $i++) {
            $matrik[$i][$i] = 1;
        }

        // inisialisasi jumlah tiap kolom dan baris kriteria
        $jmlmpb = array();
        $jmlmnk = array();
        for ($i = 0; $i <= ($JmlKriteria - 1); $i++) {
            $jmlmpb[$i] = 0;
            $jmlmnk[$i] = 0;
        }

        // menghitung jumlah pada kolom kriteria tabel perbandingan berpasangan
        for ($x = 0; $x <= ($JmlKriteria - 1); $x++) {
            for ($y = 0; $y <= ($JmlKriteria - 1); $y++) {
                $value        = $matrik[$x][$y];
                $jmlmpb[$y] += $value;
            }
        }

        // menghitung jumlah pada baris kriteria tabel nilai kriteria
        // matrikb merupakan matrik yang telah dinormalisasi
        for ($x = 0; $x <= ($JmlKriteria - 1); $x++) {
            for ($y = 0; $y <= ($JmlKriteria - 1); $y++) {
                $matrikb[$x][$y] = $matrik[$x][$y] / $jmlmpb[$y];
                $value    = $matrikb[$x][$y];
                $jmlmnk[$x] += $value;
            }

            // nilai priority vektor
            $pv[$x]     = $jmlmnk[$x] / $JmlKriteria;

            // memasukkan nilai priority vektor ke dalam tabel pv_kriteria dan pv_alternatif
            if ($jenis == 'kriteria') {
                $id_kriteria = getKriteriaID($x);
                inputKriteriaPV($id_kriteria->id, $pv[$x]);
            }
            // else {
            //     $id_kriteria    = getKriteriaID($jenis - 1);
            //     $id_alternatif    = getAlternatifID($x);
            //     inputAlternatifPV($id_alternatif, $id_kriteria->id, $pv[$x]);
            // }
        }

        $eigenvektor = getEigenVector($jmlmpb, $jmlmnk, $JmlKriteria);
        $consIndex   = getConsIndex($jmlmpb, $jmlmnk, $JmlKriteria);
        $consRatio   = getConsRatio($jmlmpb, $jmlmnk, $JmlKriteria);

        $dataView = [
            'title' => 'Perbandingan Kriteria',
            'matrik' => $matrik,
            'jmlmpb' => $jmlmpb,
            'matrikb' => $matrikb,
            'jmlmnk' => $jmlmnk,
            'pv' => $pv,
            'eigenvektor' => $eigenvektor,
            'consIndex' => $consIndex,
            'consRatio' => $consRatio,
            'n' => $JmlKriteria,

        ];

        // return response()->json($dataView);
        return view('perbandingan-kriteria.hasil', $dataView);
    }
}
