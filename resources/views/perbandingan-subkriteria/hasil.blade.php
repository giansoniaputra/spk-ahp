        ...
        <section class="content">
            ...
            <table class="ui collapsing celled blue table table-sm">
                <thead>
                    <tr>
                        <th>Sub Kriteria</th>
                        @for ($i = 0; $i <= ($n - 1); $i++) <th>{{ getSubKriteriaNama($i, $kriteria_id)->keterangan }}</th>
                            @endfor
                    </tr>
                </thead>
                <tbody>
                    @for ($x = 0; $x <= ($n - 1); $x++) <tr>
                        <td>
                            {{ getSubKriteriaNama($x, $kriteria_id)->keterangan }}
                        </td>
                        @for ($y = 0; $y <= ($n - 1); $y++) <td>
                            {{ round($matrik[$x][$y], 5) }}
                            </td>
            ...
            <table class="ui celled red table">
                ...
                <tbody>
                    @for ($x = 0; $x <= ($n - 1); $x++) <tr>
                        <td>{{ getSubKriteriaNama($x, $kriteria_id)->keterangan }}</td>
